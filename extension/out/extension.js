...
const vgaSimulatorProvider_1 = require("./vgaSimulatorProvider");
const verilogParser_1 = require("./verilogParser");
const verilatorSimulator_1 = require("./verilatorSimulator");
function activate(context) {
    console.log('VGA Simulator extension is now active!');
    // Create VGA simulator provider
    const vgaSimulatorProvider = new vgaSimulatorProvider_1.VGASimulatorProvider(context.extensionUri);
    const verilogParser = new verilogParser_1.VerilogParser();
    const verilatorSimulator = new verilatorSimulator_1.VerilatorWASMSimulator(context.extensionUri);
    // Register the webview provider
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('vga-simulator-view', vgaSimulatorProvider));
    // Register commands
    let openSimulatorCommand = vscode.commands.registerCommand('vga-simulator.openSimulator', async () => {
        const panel = vscode.window.createWebviewPanel('vgaSimulator', 'VGA Simulator', vscode.ViewColumn.Two, {
            enableScripts: true,
...
function getWebviewContent(webview, extensionUri, vgaSignals, simulator, compilationSuccess, wasmAvailable) {
