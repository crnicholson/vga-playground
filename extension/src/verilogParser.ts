export class VerilogParser {
    parseVGASignals(verilogContent: string): any {
        const signals = {
            hsync: false,
            vsync: false,
            red: [] as number[],
    ...
    extractBitWidth(line: string): number[] {
    ...
    parseModulePorts(verilogContent: string): any {
        const ports = {
            inputs: [] as Array<{name: string, width: number}>,
            outputs: [] as Array<{name: string, width: number}>,
            inouts: [] as Array<{name: string, width: number}>
        };
    ...
    parseVGAAssignments(verilogContent: string): any {
        const assignments = {
            uo_out: '',
            uio_out: '',
            uio_oe: '',
    ...
    parseModuleInstantiations(verilogContent: string): any[] {
    ...
    parseAlwaysBlocks(verilogContent: string): any[] {
        const alwaysBlocks: any[] = [];
        const lines = verilogContent.split('\n');
