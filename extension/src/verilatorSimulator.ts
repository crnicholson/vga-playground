...
export interface VerilatorSimulator {
    compile(verilogCode: string): Promise<boolean>;
    simulate(clockCycles: number): Promise<VGASignal>;
    reset(): void;
    getFrame(): Promise<VGAFrame>;
}
...
export class VerilatorWASMSimulator implements VerilatorSimulator {
    ...
    private async loadVerilatorWASM(): Promise<void> {
        try {
            ...
            this.wasmModule = {
                ...
                compile: (codePtr: number) => 1, // Success
                simulate: (cycles: number) => 0, // Return pointer
    ...
    async compile(verilogCode: string): Promise<boolean> {
    ...
    async simulate(clockCycles: number): Promise<VGASignal> {
        if (this.wasmModule && this.isLoaded) {
    ...
    async getFrame(): Promise<VGAFrame> {
        if (this.wasmModule && this.isLoaded) {
    ...
    private allocateString(str: string): number {
        if (!this.wasmModule || !this.memory) return 0;
    ...
    private extractVGASignals(resultPtr: number): VGASignal {
        if (!this.memory) return this.fallbackSimulate(1);
    ...
    private extractFrameData(framePtr: number): Uint8ClampedArray {
        if (!this.memory) return new Uint8ClampedArray(640 * 480 * 4);
    ...
    private fallbackCompile(verilogCode: string): boolean {
    ...
    private fallbackSimulate(clockCycles: number): VGASignal {
    ...
    private fallbackGetFrame(): VGAFrame {
        const width = 640;
        const height = 480;
