export class VerilogExpressionEvaluator {
    private signals: Map<string, any>;
    private state: any;

    constructor(signals: Map<string, any>, state: any) {
    ...
    private evaluateConcatenation(concatExpr: string): number {
    ...
    private getBitWidth(expression: string): number {
    ...
    private getSignalBit(signalName: string, bitIndex: number): number {
    ...
    private getSignalValue(signalName: string): number {
        // Check if it's a state variable
        if (signalName === 'pix_x' && this.state.pix_x !== undefined) {
