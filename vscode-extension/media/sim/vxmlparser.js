...
class VerilogXMLParser {
    ...
    visit_module_files(node) {
        node.children.forEach((n) => {
            if (n.obj) {
                const file = this.files[n.obj.id];
                if (file)
                    file.isModule = true;
            }
        });
    ...
    visit_cells(node) {
        ...
        const hier = node.children[0].obj;
        if (hier != null) {
    ...
    visit_extend(node) {
        const unop = this.__visit_unop(node);
    ...
    parse(xmls) {
        (0, xml_1.parseXMLPoorly)(xmls, this.xml_open.bind(this), this.xml_close.bind(this));
        this.cur_node = null;
        this.run_deferred();
    }
}
